...
class ExitRoutingService:
    ...
    async def apply(self, route: Route, origin: Coordinates, destination: Coordinates, preferences: RoutePreferences) -> Route:
        ...
        origin_access = await self._replace_walking_leg(route, first_transit - 1, origin, route.legs[first_transit].from_location, is_origin=True, preferences=preferences)
        destination_access = await self._replace_walking_leg(route, last_transit + 1, destination, route.legs[last_transit].to_location, is_origin=False, preferences=preferences)
        ...
        route.exit_routing = ExitRoutingMetadata(
            ...
            explanation="Station exits were selected using walking-network duration." if origin_access or destination_access else None,
    ...
    async def _replace_walking_leg(self, route: Route, index: int, endpoint: Coordinates, station_name: str, *, is_origin: bool, preferences: RoutePreferences) -> StationAccess | None:
        ...
        return StationAccess(station_id=selected["station_id"], station_name=selected["station_id"], exit_id=selected["id"], exit_name=exit_name, lat=selected["lat"], lon=selected["lon"])
